...
- **派生类的非继承函数**：
  派生类自己新增的、不是从基类继承来的成员函数，不能通过基类指针直接调用。编译时直接爆错，提示没有这个成员函数。

- **虚函数表在内存中的什么位置？**
  通常位于程序的`只读数据段(.rodata)`，由编译器在编译期生成
...
- 菱形继承：一个类从两个父类继承，并且父类继承了同一个基类，形成菱形结构
- 虚继承：通过在继承时使用`virtual`关键字，告诉编译器只创建一个共享的基类实例，解决二义性问题
    - 底层原理：虚基类表和虚基类指针，编译器会在派生类对象中添加一个指向唯一基类实例的指针，确保所有路径访问同一实例
    - 只有B、C虚继承时，他们各自有一个A类的对象，B、C内部通过虚基类表和虚基类指针访问各自的A类对象，有D类时，也只有一个A类对象，D中的B、C通过各自的虚基类表和虚基类指针访问同一个A类对象
