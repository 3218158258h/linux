...
> 
> 🔧 **回答：** 共享进程pid、全局变量、堆区、文件描述符、环境变量、加载的动/静态库，不共享栈区、寄存器、线程id、调度优先级、信号掩码、错误码等。嵌入式Linux推荐多线程(资源省):主线程事件循环+工作线程处理耗时任务。线程间用mutex+条件变量同步。
...
**new的底层原理：**
1. 计算 Student 对象需要多少字节
2. 从堆上申请这块内存（调用 operator new，底层是malloc）
3. 在这块内存上调用 Student 的构造函数
4. 返回对象的地址

**函数分配栈的底层原理：**
每个函数的栈帧包括：

1. 返回地址（不是返回值，是函数调用返回时继续执行的地址）      

2. 保存的旧 RBP（上一个函数栈帧的基准位置，所以也叫 基址指针/帧指针）
            ← 当前 RBP 指向这里
3. 局部变量
...
- 文件任意访问（可以直接p[offset]访问文件内容指定内容）
- 大文件处理(只有访问的页才加载)
- 进程间共享(MAP_SHARED)
...
大文件、随机访问、频繁访问不同位置：用 mmap

**read 和 mmap 的区别：**
read 有两类开销：
1. 文件页缓存未命中：内核发起磁盘 IO，把文件页读入 page cache。
2. 用户 buf 未分配物理页：copy_to_user 时可能触发缺页异常。
mmap 的首次访问通常触发缺页异常：
1. 如果文件页已经在 page cache，只需要建立页表映射，这是 minor page fault。
2. 如果文件页不在 page cache，需要先读磁盘再建立映射，这是 major page fault。
---
