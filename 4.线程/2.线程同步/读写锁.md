...
- `attr`：读写锁的属性设置，`NULL` 表示使用默认属性，分为两类：
   - 进程共享属性：`PTHREAD_PROCESS_PRIVATE`（默认）表示锁只能在同一进程内使用，`PTHREAD_PROCESS_SHARED` 表示锁可以在不同进程间使用
   - 读写优先级属性：`PTHREAD_RWLOCK_DEFAULT_NP`：默认（读优先）
                     `PTHREAD_RWLOCK_PREFER_READER_NP`：读优先
                     `PTHREAD_RWLOCK_PREFER_WRITER_NP`：写优先，等待写锁时无法获取读锁
                     `PTHREAD_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP`：写优先且同一线程不能连续获取写锁
#### 返回值
- 成功：返回 `0`
