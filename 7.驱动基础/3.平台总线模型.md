...
### 设备树下的平台总线模型
- 通过设备树描述硬件资源，平台总线通过 `of_match_table` 与设备树节点的 `compatible` 属性进行匹配，匹配成功后在 `probe` 中获取资源并初始化设备。

#### 获取硬件资源（一般在probe函数中获取）
1. **通过 `of_match_device` 获取设备节点**  
```c
inline struct device_node *of_find_node_by_path(const char *path)
```

2. **通过 `of_property_read_u32_array` 获取reg 属性**  
```c
int of_property_read_u32_array(struct device_node *np, const char *propname,
                               u32 *out_values, size_t sz);
```

3. **通过 `of_iomap` 映射物理地址**  
- 获取reg属性(物理地址)后，通过`of_iomap`映射为虚拟地址，以便驱动访问寄存器
```c
void __iomem *of_iomap(struct device_node *np, const char *propname);
```

4. **常规流程**
- 获取到硬件资源后就可以进行正常的驱动流程，如注册字符设备(号)、完善 `file_operations` 结构体、生成设备节点等。
