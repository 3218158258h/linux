...
```
**加载模块**
- 用insmod或者modprobe(推荐)，后者是对前者的封装
把驱动模块复制到开发板后执行以下命令
`insmod xxxxxx.ko`
...
**核心概念**
`modprobe` 最大的优势是：**加载模块时会自动查找并加载它依赖的其他模块**，而 `insmod` 只能手动加载单个模块，不管依赖。

**`modprobe`用法**
所有 `modprobe` 操作都需要**root权限**（加 `sudo`），核心语法：
```bash
modprobe [选项] 模块名//无需添加.ko后缀
```

**加载内核模块（最常用）**
```bash
# 加载名为usbcore的USB核心模块（会自动加载依赖的模块）
sudo modprobe usbcore

# 加载时传递参数（比如给vfat模块设置编码参数）
sudo modprobe vfat iocharset=utf8 codepage=936
```
**卸载内核模块**
```bash
# 卸载模块（会自动卸载依赖该模块且无其他使用的模块）
sudo modprobe -r usbcore

# 强制卸载（谨慎用，可能导致系统异常）
sudo modprobe -rf usbcore
```

**查看模块信息/依赖**
```bash
# 查看模块的依赖关系
sudo modprobe --show-depends usbcore

# 列出已加载的所有模块（等价于lsmod）
modprobe -l  # 或直接用lsmod更直观
```

**其他实用选项**
- `-l`：列出系统中所有可用的内核模块（路径通常在 `/lib/modules/$(uname -r)/`）
- `-n`：模拟执行（只显示要执行的操作，不实际加载/卸载）
  ```bash
  sudo modprobe -n usbcore  # 看看加载usbcore会做什么，不真加载
  ```
- `-v`：显示详细执行过程（调试用）
  ```bash
  sudo modprobe -v usbcore  # 输出加载过程的详细日志
  ```

#### 关键注意事项
1. **模块名不带后缀**：内核模块文件是 `.ko` 或 `.ko.xz` 格式（比如 `usbcore.ko.xz`），但用 `modprobe` 时只写模块名（`usbcore`）即可，系统会自动查找对应文件。
2. **模块路径**：`modprobe` 会读取 `/lib/modules/$(uname -r)/modules.dep` 文件（由 `depmod` 生成），这个文件记录了所有模块的依赖关系，若手动添加模块后需执行 `sudo depmod -a` 更新。
3. **区别于insmod/rmmod**：
   - `insmod`：只能加载指定路径的单个模块，不处理依赖（比如 `sudo insmod /lib/modules/$(uname -r)/kernel/drivers/usb/core/usbcore.ko`）；
   - `rmmod`：只能卸载单个模块，不处理依赖；
   - `modprobe`：是更友好的封装，优先用它。


#### 总结
1. `modprobe` 是Linux管理内核模块的核心命令，核心优势是**自动处理模块依赖**；
2. 常用操作：`sudo modprobe 模块名`（加载）、`sudo modprobe -r 模块名`（卸载）；
3. 操作需root权限，模块名不带 `.ko` 后缀，优先于 `insmod/rmmod` 使用。
...
**5.printk**
- 运行在内核态
- 默认输出到内核环形缓冲区（`/dev/kmsg`），可通过`dmesg`命令查看，而非直接打印到终端。
- 只有日志级别**高于**系统控制台的“默认日志级别”（`/proc/sys/kernel/printk` 第一个值），才会直接输出到控制台
- 省略级别宏时，默认用 `KERN_DEFAULT`（等价于KERN_NOTICE）：

**函数原型**
```c
#include <linux/printk.h>
// 基础格式：日志级别 + 格式化字符串 + 参数
printk(KERN_INFO "驱动加载成功：设备号=%d\n", dev_num);
```  

**日志级别（优先级从高到低）**
| 级别宏        | 数值 | 含义                 | 典型用途                     |
|---------------|------|----------------------|------------------------------|
| KERN_EMERG    | 0    | 紧急情况（系统崩溃） | 内核panic、硬件致命错误      |
| KERN_ALERT    | 1    | 必须立即处理         | 严重资源耗尽（如内存耗尽）   |
| KERN_CRIT     | 2    | 临界错误             | 硬件/驱动严重错误            |
| KERN_ERR      | 3    | 普通错误             | 驱动初始化失败、设备操作失败 |
| KERN_WARNING  | 4    | 警告                 | 非致命问题（如参数不合法）   |
| KERN_NOTICE   | 5    | 注意（默认级别）     | 正常但需关注的事件（如加载驱动） |
| KERN_INFO     | 6    | 普通信息             | 驱动加载/卸载、状态提示      |
| KERN_DEBUG    | 7    | 调试信息             | 开发调试（如函数进入/退出）  |

**查看方法**
1. **dmesg命令（最常用）**：查看内核环形缓冲区的所有日志
   ```bash
   dmesg  # 查看全部
   dmesg | grep "CAN"  # 过滤关键词
   dmesg -w  # 实时监控新的内核日志（类似tail -f）
   ```
2. **查看系统日志文件**：
   ```bash
   cat /var/log/kern.log  # 大部分Linux发行版（如Ubuntu）
   cat /var/log/messages  # 部分系统（如CentOS）
   ```
3. **调整控制台日志级别**：让低级别日志也输出到控制台
   ```bash
   # 临时设置：控制台显示所有级别（0-7）的日志
   sudo echo "7 4 1 7" > /proc/sys/kernel/printk
   ```

##################################################################################################

6. 读操作函数
在 Linux 内核中，对通过 `ioremap()` 映射的 I/O 内存（即设备寄存器）进行**读操作**时，**不能直接解引用指针**（如 `*(volatile u32 *)addr`），而应使用内核提供的专用 I/O 读函数。这些函数确保：
- 正确的内存访问语义（如禁止编译器优化、插入内存屏障）；
- 跨架构兼容性（x86、ARM、RISC-V 等行为一致）；
- 访问宽度明确（8/16/32/64 位）。

**常用读操作函数**
| 函数原型 | 说明 |
|--------|------|
| `u8 readb(const volatile void __iomem *addr);` | 从 I/O 地址读取 **1 字节**（8 位） |
| `u16 readw(const volatile void __iomem *addr);` | 读取 **2 字节**（16 位，小端序） |
| `u32 readl(const volatile void __iomem *addr);` | 读取 **4 字节**（32 位） |
| `u64 readq(const volatile void __iomem *addr);` | 读取 **8 字节**（64 位） |
> ✅ 所有函数参数类型为 `void __iomem *`，这是 `ioremap()` 返回的类型。

```c
#define REG_STATUS_OFFSET 0x00
#define REG_DATA_OFFSET   0x04
void __iomem *base = ioremap(0x48000000, 0x1000);
if (!base) {
    pr_err("ioremap failed\n");
    return -ENOMEM;
}
/* 读取状态寄存器（32 位） */
u32 status = readl(base + REG_STATUS_OFFSET);
/* 读取数据寄存器（32 位） */
u32 data = readl(base + REG_DATA_OFFSET);
/* 读取一个 8 位控制标志 */
u8 ctrl = readb(base + 0x08);
pr_info("Status=0x%x, Data=0x%x, Ctrl=0x%x\n", status, data, ctrl);
iounmap(base);
```
...
| 函数 | 用途 |
|------|------|
| `readsb(addr, buf, count)` | 从 I/O 地址连续读取 `count` 个字节到 `buf` |
| `readsw(addr, buf, count)` | 连续读取 `count` 个 16 位字 |
| `readsl(addr, buf, count)` | 连续读取 `count` 个 32 位字 |
适用于 FIFO 或 DMA 缓冲区读取。
```c
u32 buffer[16];
readsl(base + FIFO_REG, buffer, 16); // 读 16 个 32-bit 数据
```

##################################################################################################

7.写操作函数
在 Linux 内核中，对通过 `ioremap()` 映射的 I/O 内存（如设备寄存器）进行**写操作**时，必须使用内核提供的专用写函数，而**不能直接解引用指针赋值**。这些函数确保：
- 访问具有正确的内存语义（如禁止编译器重排序、插入写屏障）；
- 跨 CPU 架构（x86、ARM、RISC-V 等）行为一致；
- 明确指定写入宽度（8/16/32/64 位）；
- 保证写操作真正到达硬件（尤其在 uncached 映射下）。
...
| 函数原型 | 说明 |
|--------|------|
| `void writeb(u8 value, volatile void __iomem *addr);` | 向 I/O 地址写入 **1 字节**（8 位） |
| `void writew(u16 value, volatile void __iomem *addr);` | 写入 **2 字节**（16 位） |
| `void writel(u32 value, volatile void __iomem *addr);` | 写入 **4 字节**（32 位） |
| `void writeq(u64 value, volatile void __iomem *addr);` | 写入 **8 字节**（64 位） |
> ✅ 参数 `addr` 必须是 `ioremap()` 返回的 `void __iomem *` 类型。
```c
#define REG_CTRL_OFFSET  0x00
#define REG_DATA_OFFSET  0x04
#define REG_CMD_OFFSET   0x08
void __iomem *base = ioremap(0x48000000, 0x1000);
/* 写入控制寄存器（32 位） */
writel(0x00000001, base + REG_CTRL_OFFSET);
/* 写入数据（32 位） */
writel(0x12345678, base + REG_DATA_OFFSET);
/* 写入一个 8 位命令 */
writeb(0xA5, base + REG_CMD_OFFSET);
iounmap(base);
```
...
| 函数 | 说明 |
|------|------|
| `writesb(addr, buf, count)` | 从 `buf` 写 `count` 个字节到 I/O 地址 |
| `writesw(addr, buf, count)` | 写 `count` 个 16 位字 |
| `writesl(addr, buf, count)` | 写 `count` 个 32 位字 |
示例：向 FIFO 寄存器写入一包数据
```c
u32 tx_data[] = {0x11223344, 0x55667788, 0x99AABBCC};
writesl(base + TX_FIFO_REG, tx_data, ARRAY_SIZE(tx_data));
```
